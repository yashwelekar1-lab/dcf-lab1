...
export default function IntelligenceStoryIntro() {const [progress, setProgress] = useState(0);

useEffect(() => {const handleScroll = () => {const section = document.getElementById("intelligence-story");
...
const centerTextProgress = Math.min(Math.max((progress - 0.18) / 0.40,0),1);
...
const circleProgress = Math.min(Math.max((progress - 0.18) / 0.40,0),1);
...
const backgroundProgress = Math.min(Math.max((progress - 0.70) / 0.30,0),1);
